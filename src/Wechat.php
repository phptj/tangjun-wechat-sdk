...
class Wechat
{
     ...
     * @var unknown
     */
    const TAG_ADDUSER_URL = "/tags/members/batchtagging?";
    const TAG_DELETEUSER_URL = "/tags/members/batchuntagging?";
    ...
    public function getRevPic(){
        if (isset($this->_receive['PicUrl']))
            return array(
                'openid'=>$this->_receive['FromUserName'],
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @return string content
     */
    private function http_post_by_file($url,$path,$post_file=false){
        $oCurl = curl_init();
        if(stripos($url,"https://")!==FALSE){
            curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
            curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
        ...
        $postArray = ['media' => new \CURLFile($path)];
        curl_setopt($oCurl, CURLOPT_URL, $url);
        curl_setopt($oCurl, CURLOPT_RETURNTRANSFER, 1 );
        curl_setopt($oCurl, CURLOPT_POST,true);
        curl_setopt($oCurl, CURLOPT_POSTFIELDS,$postArray);
        $sContent = curl_exec($oCurl);
        $aStatus = curl_getinfo($oCurl);
        curl_close($oCurl);
        if(intval($aStatus["http_code"])==200){
     ...
     * 给用户加分组标签
     */
    public function addUserTagging($data)
    {
        return self::userTagging($data, self::TAG_ADDUSER_URL);
    }
    /**
     * 删除用户分组标签
     */
    public function deleteUserTagging($data)
    {
        return self::userTagging($data, self::TAG_DELETEUSER_URL);
    }
    /**
     * 添加和删除分组标签
     * @param unknown $data
     * @param unknown $url
     */
    public function userTagging($data,$url)
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.$url.'access_token='.$this->access_token, $data);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function addConditionalMenu($data){
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::MENU_ADDCONDITIONAL_URL.'access_token='.$this->access_token, $data);
    ...
    public function delConditionalMenu($data){
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::MENU_DELCONDITIONAL_URL.'access_token='.$this->access_token, $data);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return bool|mixed
     */
    public function uploadMediaNew($path, $type){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post_by_file(self::API_URL_PREFIX.self::MEDIA_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type,$path,true);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return boolean|array
     */
    public function uploadForeverMediaNew($data, $type,$is_video=false,$video_info=array()){
        if (!$this->access_token && !$this->checkAuth()) return false;
        //#TODO 暂不确定此接口是否需要让视频文件走http协议
        //如果要获取的素材是视频文件时，不能使用https协议，必须更换成http协议
        //$url_prefix = $is_video?str_replace('https','http',self::API_URL_PREFIX):self::API_URL_PREFIX;
        //当上传视频文件时，附加视频文件信息
        if ($is_video) $data['description'] = self::json_encode($video_info);
        $result = $this->http_post_by_file(self::API_URL_PREFIX.self::MEDIA_FOREVER_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type,$data,true);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return bool|mixed
     */
    private function getQRCodeJson($data){
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                ...
                self::setCache($authname,null);
                @file_put_contents('/tmp/getQRCode.log',date('Y-m-d H:i:s').':'.var_export($json,true),FILE_APPEND);
                return false;
            }
            return $json;
        }
